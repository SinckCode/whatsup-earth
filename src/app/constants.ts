...
interface Disaster {
  ...
  unitYear: string;
  unitCountry: string;
  eonetCategorySlug: string;
  eonetCategoryId?: number;
}
...
export const DISASTERS_BY_KEY: Record<string, Disaster> = DISASTERS.reduce((acc, d) => {
  acc[d.key] = d;
  return acc;
}, {} as Record<string, Disaster>);

// Helper para resolver un desastre desde la URL
export function resolveDisaster(key: string): Disaster {
